...
//var barcode = '01036608121415671728111410141123EF'    ;
//answer = parseBarcode(barcode);
//console.log(answer);

function readFile() {
    try {
        const data = fs.readFileSync('D:/OneDrive/Jim - Malue Ltd/OneDrive - Malue Ltd/Malue Resources/MedicalProducts/VascularAccesProductsAndScan/products.txt', 'utf8');
...
function setupOutputFile() {
    try {
        fs.writeFileSync('D:/OneDrive/Jim - Malue Ltd/OneDrive - Malue Ltd/Malue Resources/MedicalProducts/VascularAccesProductsAndScan/file.txt', '', 'utf8');
        //console.log("File written successfully!");
    } catch (err) {
        console.error("Error writing file:", err);
    }
}
function addToFile(outputLine) {
    try {
        fs.appendFileSync('D:/OneDrive/Jim - Malue Ltd/OneDrive - Malue Ltd/Malue Resources/MedicalProducts/VascularAccesProductsAndScan/file.txt', outputLine+'\n');
...
lines.forEach((line, index) => {
    if(index == 0){
        ...
        var barcode = cols[2];
        try {
            parsedCode = parseBarcode(barcode);
            const gtinItem = parsedCode.parsedCodeItems.find(item => item.dataTitle === "GTIN");
            const gtinValue = gtinItem ? gtinItem.data : null;

            Gtin = gtinValue;
            output = JSON.stringify(parsedCode)
            
        } catch (err) {
            console.log("Error parsing barcode:", err)
            parsedCode = {};
        }
        addToFile(line + '\t' + Gtin + '\t' + output);
    }
