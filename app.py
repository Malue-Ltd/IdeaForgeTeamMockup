...
def generate_menu_html(structure):
    ...
    def build_tree(data):
        ...
        html_content += '<ul class="nav tree nav-item" id="side-menu">'
        for key, value in data.items():
            if isinstance(value, dict):  # If it's another dictionary, go deeper
                for sub_key, sub_value in value.items():
                    html_content += f'<li>'
                    html_content += f'<span class="caret icon"><img src="{url_for('static', filename='icons/StageBlock.png')}" alt="Stage Block icon">Stage: {sub_key}</span>'
                    html_content += '<ul class="nested">'
                    for step_key, tasks in sub_value.items():
                        html_content += f'<li><span class="caret icon"><img src="{url_for('static', filename='icons/TaskBlock.png')}" alt="Task Block icon">Step: {step_key}</span>'
                        html_content += '<ul class="nested">'
                        html_content += '<div style="border: solid black 1px">'
                        for task in tasks:
                            # html_content += '<div style="border: solid black 1px">'
                            html_content += f'<span class="leaf">Task: {task["task"]}</span>'
                            ...
                            # html_content += '</div>'
                        html_content += '</ul>'
                        html_content += '</li>'  # Close step
                        # html_content += '</div>'
                    html_content += '</ul>'  # Close stage
            html_content += '</ul>'
        html_content += '</div>'
        html_content += '</nav>'
        return html_content
    return build_tree(structure)
def generate_html_items(note_content):
    return f"""
...
"""
# Separate function to generate the full HTML document
...
# Define the route that delivers the static page
@app.route('/')
def show_menu():
    # Call the generate_menu_html function and return the result
    items_html = generate_html_items(note_content)
